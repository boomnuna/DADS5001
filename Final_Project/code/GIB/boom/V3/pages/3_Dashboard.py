...
# ── Disclaimer ────────────────────────────────────────────────────────────────
st.markdown("""
<div style="background:linear-gradient(135deg,#1c1400,#2a1f00);
            border:1px solid #f59e0b;border-left:5px solid #f59e0b;
            border-radius:10px;padding:12px 18px;margin-bottom:16px;">
  <span style="font-weight:700;color:#f59e0b;font-size:0.85rem;">⚠️ Disclaimer: </span>
  <span style="color:#fcd34d;font-size:0.83rem;">
    ข้อมูลและการวิเคราะห์นี้มีไว้เพื่อการศึกษาเท่านั้น ไม่ใช่คำแนะนำทางการเงิน
...
with tab_decision:
    ...
    if ai_result.empty:
        ...
        colors_dec = [
            ...
            for r in merged_dec["recommendation"].fillna("Hold")
        ]
        fig_dec = go.Figure(go.Bar(
            x=merged_dec["ticker"], y=merged_dec["combined_score"],
            marker_color=colors_dec,
            text=merged_dec.apply(lambda r: f"{r['combined_score']:.1f} ({r.get('recommendation','Hold')})", axis=1),
